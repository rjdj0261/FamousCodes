...
style.configure("gold.Horizontal.TProgressbar",background="black", foreground="gold")

l1=Label(root,text="STAR", foreground="gold", background="black",font="none 100")
l1.place(x=500,y=10)

l2=Label(root,text="MUSIC PLAYER", foreground="gold", background="black",font="none 70")
l2.place(x=300,y=200)

progress = Progressbar(root, orient = HORIZONTAL, length = 1300,mode = 'determinate',style="gold.Horizontal.TProgressbar")

def bar():
	progress = Progressbar(root, orient = HORIZONTAL, length = 1300,mode = 'determinate',style="gold.Horizontal.TProgressbar")
	progress.place(x=50,y=600)
	import time
	x=1
	for i in range(100):
	   	progress['value'] = x
	...
	l1.place_forget()
	l2.place_forget()
	mytkinter.Button.place_forget()

style.configure("Ttkinter.Button", foreground="gold", background="black",width=25,height=100,font="none 50")

mytkinter.Button = tkinter.Button(text="START", style="Ttkinter.Button", command=bar)

mytkinter.Button.place(x=100,y=400)

root.mainloop()

root = Tk()

root.title("Star-MP3 Player")
...
# Create Playlist Box
playlist_box = Listbox(main_frame, bg="black", fg="gold", width=60, selectbackground="white", selectforeground='black')
...
# Define tkinter.Button Images For Controls
back_btn_img = PhotoImage(file='Previous.png')
...
# Create tkinter.Button Frame
control_frame = Frame(main_frame)
control_frame.grid(row=1, column=0, pady=20)

# Create Play/Stop etc tkinter.Buttons
back_button = tkinter.Button(control_frame, image=back_btn_img, borderwidth=0, command=previous_song)
forward_button = tkinter.Button(control_frame, image=forward_btn_img, borderwidth=0, command=next_song)
play_button = tkinter.Button(control_frame, image=play_btn_img, borderwidth=0, command=play)
pause_button = tkinter.Button(control_frame, image=pause_btn_img, borderwidth=0, command=lambda: pause(paused))
stop_button = tkinter.Button(control_frame, image=stop_btn_img, borderwidth=0, command=stop)
